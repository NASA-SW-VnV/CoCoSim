...
function [verif_lus_path, nodes_list] = create_emf_verif_file(...
    ...
    filetext1 = ...
        coco_nasa_utils.LustrecUtils.adapt_lustre_text(fileread(coco_lus_fpath));
    sep_line =...
        '--******************** second file ********************';
    filetext2 = ...
        coco_nasa_utils.LustrecUtils.adapt_lustre_text(fileread(lus_file_path));
    ...
    for node_idx =1:numel(emf_nodes_names)
        ...
        if ~isempty(tokens)
            ...
            if ~strcmp(new_node_name, '')
                if UseLusi
                    main_node_struct = ...
                        coco_nasa_utils.LustrecUtils.extract_node_struct(...
                        lus_file_path, original_name, LUSTREC, LUCTREC_INCLUDE_DIR);
                else
                    main_node_struct = nodes.(node_name);
                end
                contract = coco_nasa_utils.LustrecUtils.construct_contact(...
