    ...
    if isa(ds, 'Simulink.SimulationData.Dataset')
        for i=1:numel(ds.getElementNames)
            number_of_inputs = number_of_inputs + ...
                coco_nasa_utils.LustrecUtils.getNumberOfInputsInlinedFromDataSet(ds{i}.Values, nb_steps);
        end
    ...
    elseif isa(ds, 'struct')
        ...
        for i=1:numel(ds)
            for j=1:numel(fields)
                number_of_inputs = number_of_inputs + ...
                    coco_nasa_utils.LustrecUtils.getNumberOfInputsInlinedFromDataSet(ds(i).(fields{j}), nb_steps);
            end
        end
    end
end
