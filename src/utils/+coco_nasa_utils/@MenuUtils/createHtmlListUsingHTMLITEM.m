...
function html_path = createHtmlListUsingHTMLITEM(title, items_list, html_path, model)
    ...
    if useMaterialize
        ...
        if exist(materialize_path, 'dir')
            ...
            html_text = strrep(html_text, 'href="css/materialize.css"', css_online);
        ...
        html_text = strrep(html_text, 'href="css/materialize.css"', 'href=""');
        html_text = strrep(html_text, 'src="js/materialize.min.js"', 'src=""');
    end
    % add model Info
    if nargin == 4
        html_text = strrep(html_text, '[META_INFO]', coco_nasa_utils.MenuUtils.getModelInfo(title, model));
    else
    ...
    items_text = coco_nasa_utils.MatlabUtils.strjoin(items_text, '\n');
