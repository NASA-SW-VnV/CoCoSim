...
function Values = get_random_values_InTimeSeries(time, min, max, dim, dt)
    ...
    if isBus
        ...
        Values = timeseries(...
            coco_nasa_utils.SLXUtils.get_random_values(nb_steps, min, max, dim, dt), ...
            time);
    end
end
...
%     end
    ...
    for i=1:width
        for j=1:numel(elems)
            ...
            values(i).(elems(j).Name) = coco_nasa_utils.SLXUtils.get_random_values_InTimeSeries(time, min, max, dim, dt);
        end
    end
