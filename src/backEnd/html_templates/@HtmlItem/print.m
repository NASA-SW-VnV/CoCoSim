...
function res = print(obj, level)
    ...
    % choose which printing to use
    res = print_using_html5(obj, level);
%     res = print_using_materialize(obj, level);
end

function res = print_using_html5(obj, level)
    ...
    %             end
    header =  sprintf('%s<div class="%s-text text-darken-2"><h%d>%s</h%d></div>%s\n', ...
...
function res = print_using_materialize(obj, level)
    if isempty(obj.text_color)
        Textcolor = obj.colorMap(level);
    else
        Textcolor = obj.text_color;
