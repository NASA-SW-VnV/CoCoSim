...
function  [x2, y2] = process_node_call(nodes, new_model_name, node_block_path, blk_exprs, var, node_name, x2, y2, xml_trace)
    ...
    ID = coco_nasa_utils.SLXUtils.adapt_block_name(node_fieldname);
    fcn_path = coco_nasa_utils.SLXUtils.makeBlockNameUnique(...
    ...
    if strcmp(blk_exprs.(var{1}).kind, 'statefulcall') && strcmp(blk_exprs.(var{1}).reset.resetable, 'true')
        ...
        reset_adapted = coco_nasa_utils.SLXUtils.adapt_block_name(reset_name, node_name);
        reset_path =  coco_nasa_utils.SLXUtils.makeBlockNameUnique(...
