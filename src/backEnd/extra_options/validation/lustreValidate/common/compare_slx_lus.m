...
if tests_method ~= 4
    if isempty(model_full_path)
        [status, model_full_path, ~, ~] = coco_nasa_utils.LustrecUtils.construct_EMF_model(...
...
if tests_method == 1 || tests_method == 2
    ...
    for i=1:numel(T)
        [valid_i,...
            ...
            cex_file_path_i] = ...
            coco_nasa_utils.LustrecUtils.run_comparaison(slx_file_name, ...
            lus_file_path,...
            main_node_name, ...
            T(i),...
            output_dir,...
...
if (tests_method == 3)
    [status, emf_model_path] = coco_nasa_utils.LustrecUtils.construct_EMF_verif_model(slx_file_name,...
...
elseif (tests_method == 4) %4- Prove LUS1 <=> LUS2.
    ...
    [status, emf_model_path, emf_path, EMF_trace_xml] = coco_nasa_utils.LustrecUtils.construct_EMF_model(...
        lus_file_path, main_node_name, output_dir);
    if status
        return;
    end
    ...
    [verif_lus_path, nodes_list] = coco_nasa_utils.LustrecUtils.create_emf_verif_file(...
        lus_file_path,...
        coco_lus_fpath,...
        emf_path,...
        EMF_trace_xml, ...
    ...
    for i=1:numel(nodes_list)
        ...
        [valid_i, IN_struct_i] = ...
            coco_nasa_utils.Kind2Utils.extractKind2CEX(...
