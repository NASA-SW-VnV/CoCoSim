function [ lustre_nodes , open_list, abstractedNodes] = getExternalLibrariesNodes( external_libraries , backend )
[ lustre_nodes, open_list, abstractedNodes ] = recursive_call( external_libraries, {} ,backend);
...
function [ lustre_nodes, open_list, abstractedNodes ] = recursive_call( external_libraries, already_handled, backend )
...
for i=1:numel(external_libraries)
    ...
    try
        ...
        [node, external_nodes_i, opens, abstracts] = fun_handle(backend);
    catch
...
[ additional_code, additional_open_list, additional_abstractedNodes ] = recursive_call( additional_nodes, already_handled,backend );
