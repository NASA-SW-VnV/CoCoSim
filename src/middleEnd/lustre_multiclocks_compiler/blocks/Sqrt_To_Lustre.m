    ...
    methods
        ...
        function options = getUnsupportedOptions(obj, varargin)
            ...
            if ~isempty(blk.OutMax) || ~isempty(blk.OutMin)
                options{numel(options) + 1} = sprintf('The minimum/maximum value is not support in block %s', blk.Origin_path);
            end
            if strcmp(blk.SaturateOnIntegerOverflow, 'on')
                options{numel(options) + 1} = sprintf('The Saturate on integer overflow option is not support in block %s', blk.Origin_path);
            end
            if strcmp(blk.AlgorithmType, 'Newton-Raphson')
                options{numel(options) + 1}  = sprintf('Option Newton-Raphson is not supported in block %s', ...
                    blk.Origin_path);
            end
            obj.addUnsupported_options(options);
        end
    end
