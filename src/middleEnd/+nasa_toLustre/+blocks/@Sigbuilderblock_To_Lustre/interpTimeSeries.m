function [codeAst, vars] = interpTimeSeries(output,time_array, ...
    ...
    for i=1:numel(time_array)-1
        if time_array(i) == time_array(i+1)
            ...
            lowerCond = nasa_toLustre.lustreAst.BinaryExpr(nasa_toLustre.lustreAst.BinaryExpr.GTE, ...
                curTime, ...
                astTime{i}, [], LusBackendType.isLUSTREC(lus_backend), epsilon);
            upperCond = nasa_toLustre.lustreAst.BinaryExpr(nasa_toLustre.lustreAst.BinaryExpr.LT, ...
                curTime, ...
                astTime{i+1}, [], LusBackendType.isLUSTREC(lus_backend), epsilon);

            conds{end+1} = nasa_toLustre.lustreAst.BinaryExpr(nasa_toLustre.lustreAst.BinaryExpr.AND, lowerCond, upperCond);
            if interpolate
                thens{end+1} = ...
