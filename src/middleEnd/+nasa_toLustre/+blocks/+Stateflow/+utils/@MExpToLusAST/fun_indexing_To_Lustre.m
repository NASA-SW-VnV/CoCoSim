function [code, exp_dt] = fun_indexing_To_Lustre(BlkObj, tree, parent, blk,...
    ...
    switch tree_ID
        case  {'acos', 'acosh', 'asin', 'asinh', 'atan', ...
            ...
            [code, exp_dt] = nasa_toLustre.blocks.Stateflow.utils.MExpToLusAST.mathFun_To_Lustre(BlkObj, tree, parent, blk,...
                data_map, inputs, expected_dt, isSimulink, isStateFlow, isMatlabFun);
        ...
        case {'ceil', 'floor', 'round', 'fabs', ...
            ...
            [code, exp_dt] = nasa_toLustre.blocks.Stateflow.utils.MExpToLusAST.convFun_To_Lustre(BlkObj, tree, parent, blk,...
                data_map, inputs, expected_dt, isSimulink, isStateFlow, isMatlabFun);
...
function code = parseOtherFunc(obj, tree, parent, blk, data_map, inputs, expected_dt, isSimulink, isStateFlow, isMatlabFun)
    ...
    if (isStateFlow || isMatlabFun) && data_map.isKey(tree.ID)
        %Array Access
        code = nasa_toLustre.blocks.Stateflow.utils.MExpToLusAST.arrayAccess_To_Lustre(obj, tree, parent, blk, ...
            data_map, inputs,  expected_dt, isSimulink, isStateFlow, isMatlabFun);
        
    elseif (isStateFlow || isMatlabFun) && SF_MF_FUNCTIONS_MAP.isKey(tree.ID)
        %Stateflow Function and Matlab Function block
        code = nasa_toLustre.blocks.Stateflow.utils.MExpToLusAST.sf_mf_functionCall_To_Lustre(obj, tree, parent, blk, ...
            data_map, inputs, expected_dt, isSimulink, isStateFlow, isMatlabFun);
