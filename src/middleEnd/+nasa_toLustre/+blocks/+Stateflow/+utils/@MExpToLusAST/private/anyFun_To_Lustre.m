function [code, exp_dt, dim] = anyFun_To_Lustre(BlkObj, tree, parent, blk,...
    ...
    if length(tree.parameters) == 1
        if n_dim > 2
            ME = MException('COCOSIM:TREE2CODE', ...
                'Function any in expression "%s" first argument is %d-dimension, more than 2 is not supported.',...
        ...
        if param2.type == 'constant'
            ...
            ME = MException('COCOSIM:TREE2CODE', ...
                'Function any in expression "%s" second argument is not a constant is not supported.',...
                tree.text);
            throw(ME);
        end
    end
    ...
    elseif param2_value == 1
        ...
        ME = MException('COCOSIM:TREE2CODE', ...
            'Function any in expression "%s" is not supported.',...
            tree.text);
        throw(ME);
    end
