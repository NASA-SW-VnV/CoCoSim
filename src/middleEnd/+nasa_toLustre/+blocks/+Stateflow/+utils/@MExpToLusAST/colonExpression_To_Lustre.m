function [code, exp_dt, dim] = colonExpression_To_Lustre(tree, args)
    ...
    if count(tree.text, ':') == 2
        if strcmp(tree.leftExp.leftExp.type, 'constant') && strcmp(tree.leftExp.rightExp.type, 'constant') && strcmp(tree.rightExp.type, 'constant')
            [left, left_dt, ~] = nasa_toLustre.blocks.Stateflow.utils.MExpToLusAST.expression_To_Lustre(...
                tree.leftExp.leftExp, args);
            [middle, middle_dt, ~] = nasa_toLustre.blocks.Stateflow.utils.MExpToLusAST.expression_To_Lustre(...
                tree.leftExp.rightExp, args);
            [right, right_dt, ~] = nasa_toLustre.blocks.Stateflow.utils.MExpToLusAST.expression_To_Lustre(...
                tree.rightExp, args);
