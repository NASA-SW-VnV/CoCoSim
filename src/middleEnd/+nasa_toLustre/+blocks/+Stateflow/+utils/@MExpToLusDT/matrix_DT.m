function [lusDT, slxDT] = matrix_DT(tree, args)
    ...
    if isLeft
        ...
        for j=1:numel(columns)
            [lusDT_i, slxDT_i] = nasa_toLustre.blocks.Stateflow.utils.MExpToLusDT.expression_DT(...
                columns(j), args);
            lusDT{j} = lusDT_i;
            slxDT{j} = slxDT_i;
        end
    else
        [lusDT, slxDT] = nasa_toLustre.blocks.Stateflow.utils.MExpToLusDT.expression_DT(rows{1}(1), ...
            args);
    end
end
