function [code, exp_dt, dim] = numFun_To_Lustre(tree, args, num)
    ...
    [x, ~, x_dim] = nasa_toLustre.blocks.Stateflow.utils.MExpToLusAST.expression_To_Lustre(...
    ...
    elseif length(tree.parameters) > 1
        for i=2:length(tree.parameters)
            [x, ~, ~] = nasa_toLustre.blocks.Stateflow.utils.MExpToLusAST.expression_To_Lustre(...
