...
function [transitionNode, external_libraries] = ...
    ...
    % This function is only called on Junctions that has no state as
    % destination.

    parentPath = fileparts(junction.Path);
    
    T = nasa_toLustre.blocks.Stateflow.utils.SF2LusUtils.orderObjects(...
        junction.OuterTransitions, 'ExecutionOrder');
    
    isDefaultTrans = false; 
    isFlowChartJunction = true;% To add termination condition as an output
    node_name = ...
        nasa_toLustre.blocks.Stateflow.utils.SF2LusUtils.getStateOuterTransNodeName(junction);
    comment = nasa_toLustre.lustreAst.LustreComment(...
        sprintf('Outer transitions of junction %s', junction.Origin_path), true);
