        ...
        function  write_code(obj, parent, blk, xml_trace, lus_backend, ~, main_sampleTime, varargin)
            ...
            if LusBackendType.isPRELUDE(lus_backend)
                ...
                elseif strcmp(type, '1/z')
                    ...
                    init_cond =nasa_toLustre.utils.SLX2LusUtils.getInitialOutput(parent, blk,...
                        blk.InitialCondition, outputDataType, nb_outputs);
                    clockInName =nasa_toLustre.utils.SLX2LusUtils.clockName(inTs/main_sampleTime(1), inTsOffset/main_sampleTime(1));
                    ...
                    if nasa_toLustre.utils.SLX2LusUtils.isIgnoredSampleTime(...
                        ...
                        cond = nasa_toLustre.lustreAst.BinaryExpr(...
                            ...
                            clockInVar, clockOutVar);
                    end
                    for i=1:length(outputs)
                        % Out = if C_out and C_in then (0.0 -> pre In) else (0.0 -> (pre Out));
                        lhs = nasa_toLustre.lustreAst.IteExpr( ...
                            cond,...
                            nasa_toLustre.lustreAst.BinaryExpr(...
                            nasa_toLustre.lustreAst.BinaryExpr.ARROW, ...
                            init_cond{i}, ...
                ...
                elseif strcmp(type, 'Copy')
                    ...
                    if nasa_toLustre.utils.SLX2LusUtils.isIgnoredSampleTime(...
                        ...
                        for i=1:length(outputs)
                            lhs = nasa_toLustre.lustreAst.IteExpr(clockOutVar, ...
                                inputs{i}, ...
                                nasa_toLustre.lustreAst.BinaryExpr(...
                                nasa_toLustre.lustreAst.BinaryExpr.ARROW, ...
                                init_cond{i}, ...
