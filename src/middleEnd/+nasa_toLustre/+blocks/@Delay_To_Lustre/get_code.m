...
function [lustre_code, delay_node_code, variables, external_libraries] = ...
    ...
    if strcmp(DelayLengthSource, 'Dialog')
        ...
        if ~coco_nasa_utils.MatlabUtils.contains(delayDataType, 'int')
    ...
    if isReset
        ...
        if coco_nasa_utils.LusBackendType.isLUSTREC(lus_backend) ...
                || coco_nasa_utils.LusBackendType.isZUSTRE(lus_backend)
    ...
    if isReset || isDelayVariable || isEnabe
        ...
        if isReset && coco_nasa_utils.LusBackendType.isJKIND(lus_backend)
