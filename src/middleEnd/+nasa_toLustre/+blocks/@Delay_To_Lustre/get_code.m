function [lustre_code, delay_node_code, variables, external_libraries] = ...
    ...
    if isReset
        ...
        codes{end + 1} = nasa_toLustre.lustreAst.LustreEq(nasa_toLustre.lustreAst.VarIdExpr(reset_var),...
            resetCode);
    ...
    if isEnabe
        ...
        enableCondition = nasa_toLustre.lustreAst.BinaryExpr(BinaryExpr.GT, ...
        ...
        for i=1:numel(u)
            ...
            lhs = nasa_toLustre.lustreAst.VarIdExpr(varName);
            variables{end + 1} = nasa_toLustre.lustreAst.LustreVar(varName, dt);
            codes{end + 1} = nasa_toLustre.lustreAst.LustreEq(lhs, ...
                nasa_toLustre.lustreAst.IteExpr(enableCondition, ...
                nasa_toLustre.lustreAst.VarIdExpr(u{i}), ...
                nasa_toLustre.lustreAst.BinaryExpr(BinaryExpr.ARROW, ...
                            x0{i}, ...
                            nasa_toLustre.lustreAst.UnaryExpr(UnaryExpr.PRE, nasa_toLustre.lustreAst.VarIdExpr(varName)))));
            %codes{end + 1} = sprintf(...
            %    '%s = if  %s then %s\n\t\t\t', ...
            %    lhs, enableCondition, u{i} );
            %codes{end + 1} = sprintf(...
            %    'else %s -> pre %s;\n\t', x0{i}, varName);
            u{i} = nasa_toLustre.lustreAst.VarIdExpr(varName);
        end
    end
    ...
    if isReset || isDelayVariable || isEnabe
        ...
        for i=1:numel(u)
            ...
            if isReset
                args{end + 1} = nasa_toLustre.lustreAst.VarIdExpr(reset_var);
            end
            if isEnabe
                args{end + 1} = enableCondition;
            end
            pre_u{i} = nasa_toLustre.lustreAst.NodeCallExpr(delay_node_name, args);
