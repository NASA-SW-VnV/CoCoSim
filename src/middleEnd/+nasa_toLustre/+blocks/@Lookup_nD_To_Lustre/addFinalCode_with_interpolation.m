function [body, vars] = addFinalCode_with_interpolation(outputs,inputs,...
    ...
    for i=1:NumberOfTableDimensions
        clipped_inputs{i} = nasa_toLustre.lustreAst.VarIdExpr(sprintf('%s_clip_input_%d',blk_name,i));
        ...
        if strcmp(ExtrapMethod,'Clip')
            ...
            body{end+1} = nasa_toLustre.lustreAst.LustreEq(clipped_inputs{i},rhs);
        else
            %body = sprintf('%s%s = %s ;\n\t', body,clipped_inputs{i},inputs{i}{1});
            body{end+1} = nasa_toLustre.lustreAst.LustreEq(clipped_inputs{i},inputs{i}{1});
        end
    end
