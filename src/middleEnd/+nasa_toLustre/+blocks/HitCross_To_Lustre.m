classdef HitCross_To_Lustre < nasa_toLustre.frontEnd.Block_To_Lustre
    ...
    methods
        ...
        function  write_code(obj, parent, blk, xml_trace, varargin)
            ...
            offset = SLX2LusUtils.num2LusExp(HitCrossingOffset, ...
            ...
            crossingCond = arrayfun(@(i) ...
                SLX2LusUtils.getResetCode(HitCrossingDirection, lus_inport_dt, inputs{i}, offset ), ...
                (1:numel(outputs)), 'UniformOutput', 0);
            crossingCondVars = cellfun(@(x) ...
                VarIdExpr(strcat(x.getId(), '_CrossCond')), outputs, 'un', 0);
            obj.addVariable(cellfun(@(x) ...
                LustreVar(x.getId(), 'bool'), crossingCondVars, 'un', 0));
            obj.addCode(arrayfun(@(i) LustreEq(crossingCondVars{i}, crossingCond{i}), ...
            ...
            if strcmp(out_lus_dt, 'bool')
                ...
                rhsVars = cellfun(@(x) ...
                    VarIdExpr(strcat(x.getId(), '_value')), outputs, 'un', 0);
                obj.addVariable(cellfun(@(x) ...
                    LustreVar(x.getId(), 'bool'), rhsVars, 'un', 0));
            end
            pre_rhsVars = cellfun(@(x) ...
                BinaryExpr(BinaryExpr.ARROW, ...
                BooleanExpr(false), ...
                UnaryExpr(UnaryExpr.PRE, x)), rhsVars, 'un', 0);
            rhsValues = arrayfun(@(i) ...
                BinaryExpr(BinaryExpr.OR, ...
                BinaryExpr(BinaryExpr.EQ, inputs{i}, offset), ...
                BinaryExpr(BinaryExpr.AND, ...
                UnaryExpr(UnaryExpr.NOT, pre_rhsVars{i}), ...
                crossingCondVars{i})), ...
                (1:numel(rhsVars)), 'UniformOutput', 0);
            obj.addCode(arrayfun(@(i) LustreEq(rhsVars{i}, rhsValues{i}), ...
                (1:numel(rhsVars)), 'UniformOutput', 0));
            
            % convert bool to output data type
            if ~strcmp(out_lus_dt, 'bool')
                [external_lib, conv_format] = ...
                    SLX2LusUtils.dataType_conversion('bool', out_lus_dt);
                if ~isempty(conv_format)
                    ...
                    rhs = cellfun(@(x) ...
                        ...
                        rhs, 'un', 0);
                    codes = arrayfun(@(i) LustreEq(outputs{i}, rhs{i}), ...
                        (1:numel(outputs)), 'UniformOutput', 0);
                    obj.addCode( codes );
                end
            end
