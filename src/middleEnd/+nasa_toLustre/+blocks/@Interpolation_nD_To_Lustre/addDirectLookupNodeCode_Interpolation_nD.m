function [body, vars] = addDirectLookupNodeCode_Interpolation_nD(...
    ...
    sol_subs_for_dim_max_clip = cell(1,NumberOfTableDimensions);
    tableSize = size(blkParams.Table);
    for j=1:NumberOfTableDimensions
        sol_subs_for_dim_max_clip{j} = ...
            nasa_toLustre.lustreAst.VarIdExpr(sprintf(...
            'max_clip_sol_subs_for_dim_%d',j));
        vars{end+1} = nasa_toLustre.lustreAst.LustreVar(...
                sol_subs_for_dim_max_clip{j},'int');
