...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [code, exp_dt, dim, extra_code] = repmatFun_To_Lustre(tree, args)
...
%    
    ...
    if isempty(Y) || (~isa(Y{1}, 'nasa_toLustre.lustreAst.RealExpr') ...
        ...
        ME = MException('COCOSIM:TREE2CODE', ...
            'Second argument in function repmat in expression "%s" should be a constant.',...
            tree.text);
        throw(ME);
    end
    if numel(Y) == 1
    ...
    elseif numel(Y) == 2
        ...
        ME = MException('COCOSIM:TREE2CODE', ...
            'Function repmat in expression "%s" second argument is %d-dimension, more than 2 is not supported.',...
            tree.text, numel(Y));
        throw(ME);
    end
    
    if (length(tree.parameters) == 2)
        code1 = repmat(X_reshp, Y);
    elseif (length(tree.parameters) == 3)
        args.expected_lusDT = 'int';
        [d, ~, ~, extra_code_i] = nasa_toLustre.utils.MExpToLusAST.expression_To_Lustre(tree.parameters(3),args);
        extra_code = MatlabUtils.concat(extra_code, extra_code_i);
        if isempty(d) || ~isa(d{1}, 'nasa_toLustre.lustreAst.IntExpr')
            ME = MException('COCOSIM:TREE2CODE', ...
                'Third argument in function repmat in expression "%s" should be a constant.',...
                tree.text);
            throw(ME);
        end
        code1 = repmat(X_reshp, Y, d{1}.value);
    else
        ME = MException('COCOSIM:TREE2CODE', ...
            'Function repmat in expression "%s" do not support more than 2 arguments.',...
            tree.text);
        throw(ME);
    end
    dim = size(code1);
