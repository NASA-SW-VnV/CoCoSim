...
function [code, exp_dt, dim, extra_code] = maxMinFun_To_Lustre(tree, args, op)
    ...
    elseif length(tree.parameters) == 2
        [x, x_dt, x_dim, extra_code_1] = nasa_toLustre.utils.MExpToLusAST.expression_To_Lustre(...
            tree.parameters(1), args);
        [y, ~, y_dim, extra_code_2] = nasa_toLustre.utils.MExpToLusAST.expression_To_Lustre(...
        ...
        extra_code = coco_nasa_utils.MatlabUtils.concat(extra_code_1, extra_code_2);
        [x, y] = nasa_toLustre.utils.MExpToLusAST.inlineOperands(x, y, tree);
        if strcmp(op, nasa_toLustre.lustreAst.BinaryExpr.LTE)
            func = strcat('_min_', x_dt);
        else
            func = strcat('_max_', x_dt);
        end
        
        args.blkObj.addExternal_libraries(strcat('LustMathLib_', func));
        
        code = cell(1, length(x));
        for i = 1:length(x)
            code{i} = nasa_toLustre.lustreAst.NodeCallExpr(func, ...
                {x{i}, y{i}});
        end
        exp_dt = x_dt;
    else
        ME = MException('COCOSIM:TREE2CODE', ...
            'Function "%s" in expression "%s" has more than 2 arguments is not supported in block %s.',...
            tree.ID, tree.text, HtmlItem.addOpenCmd(args.blk.Origin_path));
        throw(ME);
    end
end
