function [code, exp_dt, dim] = allAnyFun_To_Lustre(tree, args, op)
    ...
    [x, ~, x_dim] = nasa_toLustre.utils.MExpToLusAST.expression_To_Lustre(tree.parameters(1), args);
    n_dim = numel(x_dim);
    ...
    if length(tree.parameters) == 1
        if n_dim > 2
            ME = MException('COCOSIM:TREE2CODE', ...
                'Function in expression "%s" first argument is %d-dimension, more than 2 is not supported.',...
        ...
        if param2.type == 'constant'
            ...
            ME = MException('COCOSIM:TREE2CODE', ...
                'Function in expression "%s" second argument is not a constant is not supported.',...
                tree.text);
            throw(ME);
        end
    end
