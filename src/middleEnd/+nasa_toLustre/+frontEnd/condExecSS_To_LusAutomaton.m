...
function [body, variables_cell] =...
    ...
    if ~exist('original_node_call', 'var')
        original_node_call = LustreEq(node_outputs_withoutDT, ...
            NodeCallExpr(blk_name, node_inputs_withoutDT));
    end
    ...
    for i=1:numel(Outportfields)
        ...
        for out_idx=1:numel(outputs_i)
            ...
            variables_cell{end + 1} = LustreVar(pre_out_name, ...
                outputs_DT_i{out_idx}.getDT());
            inactiveStatement{end+1} = LustreEq(outputs_i{out_idx}, ...
                VarIdExpr(pre_out_name));
            if strcmp(OutputWhenDisabled, 'reset') && (hasActionPort || hasEnablePort)
                pre_out_str{end+1} = LustreEq(VarIdExpr(pre_out_name),...
                    InitialOutput_cell{out_idx});
            else
                pre_out_str{end+1} = LustreEq(...
                    VarIdExpr(pre_out_name),...
                    IteExpr(BinaryExpr(BinaryExpr.GT, ...
                    VarIdExpr(SLX2LusUtils.nbStepStr()),...
                    IntExpr(0)), ...
                    UnaryExpr(UnaryExpr.PRE, outputs_i{out_idx}), ...
                    InitialOutput_cell{out_idx}));
            end
        end
    end
    ...
    states{1} = AutomatonState(active_state_name, ...
        {},...
        {AutomatonTransExpr(UnaryExpr(UnaryExpr.NOT, ...
        VarIdExpr(SLX2LusUtils.isEnabledStr())),...
        true, ...
        inactive_state_name)},...
        {}, ...
        {original_node_call});
    states{2} = AutomatonState(inactive_state_name, ...
        {},...
        {AutomatonTransExpr(VarIdExpr(SLX2LusUtils.isEnabledStr()),...
        is_restart, ...
        active_state_name)},...
        {}, ...
        inactiveStatement);
    automaton{1} = LustreAutomaton(automaton_name, states);
...
function [body, variables_cell] = write_enabled_AND_triggered_action_SS(subsys, blk_name, ...
    ...
    for i=1:numel(Outportfields)
        ...
        for out_idx=1:numel(outputs_i)
            ...
            if strcmp(OutputWhenDisabled, 'reset')
                ...
                variables_cell{end + 1} = LustreVar(pre_held_name, ...
                    outputs_DT_i{out_idx}.getDT());
                pre_out_str{end+1} = LustreEq(...
                    VarIdExpr(pre_held_name),...
                    IteExpr(BinaryExpr(BinaryExpr.GT, ...
                    VarIdExpr(SLX2LusUtils.nbStepStr()),...
                    IntExpr(0)), ...
                    UnaryExpr(UnaryExpr.PRE, outputs_i{out_idx}), ...
                ...
                inactiveStatement{end+1} = ...
                    LustreEq(outputs_i{out_idx}, ...
                    VarIdExpr(pre_held_name));
            else
                inactiveStatement{end+1} = ...
                    LustreEq(outputs_i{out_idx}, ...
                    VarIdExpr(sprintf('pre_%s',out_name)));
            end
        end
    end
    %
    ...
    states{1} = AutomatonState(active_state_name, ...
        {},...
        {AutomatonTransExpr(UnaryExpr(UnaryExpr.NOT, ...
        VarIdExpr(SLX2LusUtils.isTriggeredStr())),...
        false, ...
        inactive_state_name)},...
        {}, ...
        {original_node_call});
    states{2} = AutomatonState(inactive_state_name, ...
        {},...
        {AutomatonTransExpr(VarIdExpr(SLX2LusUtils.isTriggeredStr()),...
        false, ...
        active_state_name)},...
        {}, ...
        inactiveStatement);
    automaton = LustreAutomaton(automaton_name, states);
