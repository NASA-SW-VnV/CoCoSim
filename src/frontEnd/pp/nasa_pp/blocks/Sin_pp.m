        ...
        for i=1:length(Sin_list)
            try
                ...
                if strcmp(SineType, 'Sample based')
                    [Samples, ~, status] = coco_nasa_utils.SLXUtils.evalParam(...
                        model, ...
                    ...
                    sampleTime = coco_nasa_utils.SLXUtils.getCompiledParam(Sin_list{i}, 'CompiledSampleTime');
                    Frequency = num2str(2*pi/(Samples* sampleTime(1)));
                    [Offset, ~, status] = coco_nasa_utils.SLXUtils.evalParam(...
                        model, ...
                        get_param(Sin_list{i}, 'Parent'), ...
                        Sin_list{i}, ...
