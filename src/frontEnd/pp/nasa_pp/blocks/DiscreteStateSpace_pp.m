...
function [status, errors_msg] = DiscreteStateSpace_pp(model)
    ...
    if not(isempty(dss_list))
        compiledInportDim = coco_nasa_utils.SLXUtils.getCompiledParam(dss_list, 'CompiledPortDimensions');
        ...
        for i=1:length(dss_list)
            ...
            try
                ...
                if strcmp(blocktype, 'StateSpace')
                    try
                        ST = coco_nasa_utils.SLXUtils.getModelCompiledSampleTime(model);
                        [a, ~, status] = coco_nasa_utils.SLXUtils.evalParam(...
                            model, ...
                        ...
                        [b, ~, status] = coco_nasa_utils.SLXUtils.evalParam(...
                            model, ...
                            get_param(dss_list{i},'Parent'), ...
                            dss_list{i}, ...
                            B);
